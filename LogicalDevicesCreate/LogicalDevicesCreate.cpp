...
private:
    ...
    void createLogicalDevice() {
        std::optional<uint32_t> indices = findGraphicQueueFamilies(physicalDevice_);

        vk::DeviceQueueCreateInfo queueCreateInfo;
        queueCreateInfo.setQueueFamilyIndex(indices.value());
    ...
    bool isDeviceSuitable(vk::PhysicalDevice device) {
        std::optional<uint32_t> indice = findGraphicQueueFamilies(device);
        return indice.has_value();
    }

    std::optional<uint32_t> findGraphicQueueFamilies(vk::PhysicalDevice device)
    {
